...
class ResnetPretrainedModel(nn.Module):
    def __init__(self, model_name, pretrained=True, out_indices=(1, 2, 3)):
        super().__init__()
        self.model = timm.create_model(
            model_name,
            pretrained=pretrained,
            features_only=True,
            out_indices=out_indices,  # Select layers 1, 2, 3
        )

    def forward(self, x):
        """
        return a list of with indices select as above (with image size 224)
...
class RecursiveConvNeXtBlock(nn.Module):
    def __init__(self, dim, layer_scale_init_value=1e-6):
        super().__init__()
        # 1. Depthwise Conv (Large Kernel)
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=7, padding=3, groups=dim)

        # 2. Layer Norm (Note: ConvNeXt uses LayerNorm, not GroupNorm)
        self.norm = nn.LayerNorm(dim, eps=1e-6)

        # 3. Inverted Bottleneck (Pointwise Convs)
        # Expand dim -> 4 * dim
        ...
        # 4. Layer Scale (helps stability in recursive loops)
        self.gamma = (
    ...
    def forward(self, x):
        ...
        # Prepare for Linear Layers: [b, c, h, w] -> [b, h, w, c]
...
class RecursiveFER(nn.Module):
    def __init__(
        ...
        recursive_block: Optional[Callable[[int], nn.Module]] = None,
    ):
        super().__init__()
        ...
        hidden_dim = self.get_stem_dimension()[1]

        # 2. The Loop Body
        self.recursive_block = (
            recursive_block(hidden_dim)
            if recursive_block is not None
            else RecursiveBlock(hidden_dim)
        )
        ...
        self.halt_head = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)), nn.Flatten(), nn.Linear(hidden_dim, 1)
        )

    def get_stem_dimension(self):
        input = torch.randn(1, 3, 224, 224)
        features = self.stem(input)
        if isinstance(features, (list, tuple)):
            return features[-1].shape  # Use the last (deepest) feature map
    ...
    def forward(self, x, labels=None, step_weights=None):
        ...
        h = self.stem(x)
        # If stem returns multiple feature maps, use the last one
        ...
        for step in range(self.max_steps):
            ...
            if self.use_gradient_checkpointing and self.training:
                ...
                h = checkpoint(
                    create_recursive_forward(self.recursive_block),
                    h,
                    use_reentrant=False,
                )
            else:
